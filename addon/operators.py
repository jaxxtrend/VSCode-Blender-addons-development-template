...
class TEMPLATE_OT_delete_selected(Operator):
    ...
    confirm: bpy.props.BoolProperty(
        name="Confirm", description="Confirm deletion", default=True
    )
...
class TEMPLATE_OT_duplicate_array(Operator):
    ...
    count: bpy.props.IntProperty(
        name="Count", description="Number of duplicates", default=3, min=1, max=20
    )
...
class TEMPLATE_OT_random_material(Operator):
    ...
    def execute(self, context):
        ...
        for obj in selected_objects:
            ...
            if material.node_tree:
                ...
                if principled:
                    ...
                    principled.inputs[9].default_value = random.uniform(
                        0.0, 1.0
                    )  # Roughness
        ...
        self.report(
            {"INFO"}, f"Applied random materials to {len(selected_objects)} object(s)"
        )
        return {"FINISHED"}
